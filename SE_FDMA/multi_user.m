...
for ii = 1:length(Eb_N0_dB)
    ...
    s1mat_Nfft = zeros(Nfft, length(s1mat(1, 1:end))); % 初始化用户 1 映射到 Nfft 个子载波上的信号矩阵
    s1mat_Nfft(30:30+(M_subcarrier) -1, 1:end) = s1mat; % 将用户 1 的信号映射到指定的子载波上
    ...
    % Prefixe Cyclique
    s1matcp=[s1_ifft(end-Ncp+1:end,:);s1_ifft]; % 加CP
    ...
    ycp_mat = reshape(ycp, Ncp +Nfft, []); % 转换为矩阵
    ycp_mat = ycp_mat(Ncp +1:end, :); %去除CP
