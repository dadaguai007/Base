...
close all;
clear all;
% 充分考虑了ZF和MMSE的信道均衡（考虑了信道的信道向量的使用，如何构建均衡所需的信道向量元素）
% 以及如何处理因果项和非因果项的均衡器
%% Simulation parameters
% On d閏rit ci-apr鑣 les param鑤res g閚閞aux de la simulation
...
for ii = 1:length(Es_N0_dB)
    ...
    %%
    %
    % 直接使用filter函数进行零强迫均衡，如果存在稳定的因果滤波器
    ...
    [r, p, k]=residuez(1, hc);%部分分式展开的目的是将复杂的有理函数分解为更简单的形式，方便后续计算逆滤波器的冲激响应。
    ...
    % 分子多项式：fliplr ( conj ( hc ) ) 是对信道冲激响应 hc 先取共轭，再进行左右翻转操作，得到的结果作为 MMSE 均衡器传递函数的分子多项式系数向量。
    % 分母多项式：
    % conv ( hc , fliplr ( conj ( hc ) ) ) 计算信道冲激响应 hc 与其共轭翻转后的向量的卷积，这部分对应于信道自相关的结果。
    % sig2b 是噪声功率，sigs2 是发送信号的功率，(sig2b / sigs2) * deltac 是将噪声功率与信号功率的比值乘以 deltac 向量。
    % 两者相加 conv ( hc , fliplr ( conj ( hc ) ) ) + ( sig2b / sigs2) * deltac 得到 MMSE 均衡器传递函数的分母多项式系数向量。
    % residuez 函数用于对离散时间系统的传递函数进行部分分式展开，
   
    [ r , p , k ] = residuez ( fliplr ( conj ( hc ) ) , ( conv ( hc , fliplr ( conj ( hc ) ) ) + ( sig2b / sigs2) * deltac ) ) ;
    %     % 调用ComputeRI函数计算MMSE均衡器的冲激响应
    ...
    %% ZF Contraint
    % 有约束的ZF
    Nw = 30; % C'est � optimiser
    %     % 生成托普利茨矩阵，生成的托普利茨矩阵 H 用于描述信道对信号的影响，在后续的均衡器设计中起到关键作用。
    H= toeplitz ( [hc(1) zeros(1 , Nw -1)]' , [hc , zeros(1 , Nw-1)]);

    Ry = (conj(H)*H.');%计算得到自相关矩阵 Ry，它反映了信道输出信号之间的相关性
    p = zeros(Nw+Lc - 1, 1);
    % 计算矩阵 P 并找到最优抽头位置
    P = (H.'*inv(Ry))*conj(H);
    [alpha, dopt] = max(diag(abs(P))); % 找到对角线上最大的值
    ...
    Nw = 100;
    H= toeplitz ( [hc(1) zeros(1 , Nw -1)]' , [hc , zeros(1 , Nw-1)]);
    Ry = sigs2*(conj(H)*H.') + sig2b*eye(Nw);  % 表示噪声的贡献。两者相加得到综合考虑信号和噪声的自相关矩阵
    p = zeros(Nw+Lc - 1, 1);
