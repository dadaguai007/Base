...
for i = 1:length(EbN0) % use parfor ('help parfor') to parallelize  
  ...
  trellis = poly2trellis(5,{'x4+x3+x2+1','x3+x2+1'});
  
  G2 = [1,1,1,0,1;0,1,1,0,1];
 
  bits_enc1 = mod(conv(tx_bit,G2(1,:)),2);
  bits_enc2 = mod(conv(tx_bit,G2(2,:)),2);
  ...
  rx_dec = conv_soft_e2_qpsk(rx_data,G2);
  bits_out = rx_dec(1:N);
...
% plots
figure(1)  
c = semilogy(EbN0, BER,'r-o','DisplayName','coded transmission-soft'); 
c.LineWidth =2;
ylim([1e-4 1]);
hold on
grid on
d = semilogy(EbN0,berub,'r--o','DisplayName','upper bound-soft');
d.LineWidth =2;
legend
xlabel('EbN0(dB)');
ylabel('BER');