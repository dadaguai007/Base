...
for i = 1:length(EbN0) % use parfor ('help parfor') to parallelize  
  ...
  bits_enc = conv_encode_eps4(tx_bit);
  
  % [MOD] symbol mapper
  tx_sym = modulation(bits_enc,'AMPM');
  ...
  rx_dec = conv_soft_e4_ampm(rx_data);
  bits_out = rx_dec(1:N);
...
% plots
figure(1)  
b = semilogy(EbN0, BER,'-^','DisplayName','coded transmission(system 3)'); 
b.Color = 	'#7E2F8E';
b.LineWidth =1;
ylim([1e-4 1]);
hold on
